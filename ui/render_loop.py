...
class RenderLoop:
    ...
    def _init_screens(self) -> None:
        ...
        self.screens[ScreenName.PLAYING] = PlayingScreen(
            self.config, 0, self.w, self.h
        )
        self.screens[ScreenName.PAUSED] = PauseScreen()
        self.screens[ScreenName.GAME_OVER] = GameOverScreen(
            0, self.hs_file, self.hs_list
        )
        self.screens[ScreenName.VICTORY] = VictoryScreen(
            0, self.hs_file, self.hs_list
        )

    def _get_playing_screen(self) -> PlayingScreen:
        """Return the PLAYING slot narrowed to its concrete type.

        self.screens is typed dict[ScreenName, Screen], so mypy only
        knows the generic Screen interface at each lookup — it has no
        way to know PLAYING always holds a PlayingScreen specifically.
        This cast documents and centralizes that invariant in one
        place instead of repeating it at every call site.

        Returns:
            The current PlayingScreen instance.
        """
        return cast(PlayingScreen, self.screens[ScreenName.PLAYING])
    ...
    def switch_to_screen(self, screen_name: ScreenName) -> None:
        ...
        is_pausing = (
            screen_name == ScreenName.PAUSED
            and self.current_screen == ScreenName.PLAYING
        )
        if is_pausing:
            self.playing_screen_backup = self.screens.get(
                ScreenName.PLAYING
            )

        # Leaving for the menu ends the run: the paused game must
        # never be restored afterwards, otherwise "Start Game" would
        # resume the previous party instead of starting a fresh one.
        if screen_name == ScreenName.MENU:
            self.playing_screen_backup = None

        is_resuming = (
            self.current_screen == ScreenName.PAUSED
            and screen_name == ScreenName.PLAYING
        )
        if is_resuming and self.playing_screen_backup:
        ...
        if screen_name == ScreenName.GAME_OVER:
            final_score = self._get_playing_screen().score
            self.screens[ScreenName.GAME_OVER] = GameOverScreen(
                final_score, self.hs_file, self.hs_list
            )
        ...
        if screen_name in self.screens:
            ...
            print(
                f"Warning: Screen {screen_name} not found, "
                "switching to menu"
            )
    ...
    def update(self) -> None:
        ...
        # dt is fixed for now (1/60); switch to clock-measured time if
        # frame drops become noticeable.
        dt = 1.0 / FPS
    ...
    def draw(self) -> None:
        ...
        is_paused = self.current_screen == ScreenName.PAUSED
        if is_paused and self.playing_screen_backup is not None:
